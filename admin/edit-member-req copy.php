    ...
    <form role="form" method="POST">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($fecha_registro)) {
                $updates[] = "fecha_registro='$fecha_registro'";
            }
            if (!empty($precio)) {
            ...
            if (!empty($nivel_aptitud)) {
                $updates[] = "nivel_aptitud='$nivel_aptitud'";
            }
            if (!empty($patologias)) {
                $updates[] = "patologias='$patologias'";
            }

            // Si hay campos a actualizar, construir y ejecutar la consulta
            if (!empty($updates)) {
                $qry_update = "UPDATE miembros SET " . implode(', ', $updates) . " WHERE id_usuario='$user_id'";
                        ...
                        <div class='widget-box'>
                          ...
                          <div class='widget-content'>
                            <div class='error_ex'>
                              ...
                              <a class='btn btn-warning btn-big'  href='edit-memberform.php?id=$user_id'>Atrás</a> 
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div class='row-fluid'>
                  <div class='span12'>
                    <div class='widget-box'>
                      <div class='widget-title'>
                        <span class='icon'> <i class='fas fa-info'></i> </span>
                        <h5>Mensaje de Información</h5>
                      </div>
                      <div class='widget-content'>
                        <div class='error_ex'>
                          <h1>No hay cambios</h1>
                          <h3>No se realizaron cambios para actualizar.</h3>
                          <p>Por favor modifique al menos un campo para continuar.</p>
                          <a class='btn btn-warning btn-big'  href='edit-memberform.php?id=$user_id'>Atrás</a> 
                        </div>
                      </div>
                    </div>
